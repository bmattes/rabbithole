...
const REVIEWER_PERSONAS: Record<string, PersonaTier> = {
  ...
  // Wikidata-backed aliases for mb_* domains
  get rock()    { return (this as any).mb_rock },
  get hiphop()  { return (this as any).mb_hiphop },
  get pop()     { return (this as any).mb_pop },
  get rnb()     { return (this as any).mb_rnb },
  get country() { return (this as any).mb_country },
  history: {
...
async function evaluatePuzzle(
  ...
  const domainNotes: Record<string, string> = {
    ...
    tv: 'IMPORTANT: For the TV shows category, TV networks (e.g. "HBO", "AMC", "Netflix"), genres (e.g. "drama", "sitcom"), and fictional characters (e.g. "Walter White", "Tony Soprano") are all VALID bridge nodes connecting shows — do NOT flag these as wrong_domain or abstract. Fictional characters are different from actors — both are valid.',
    ...
    food: 'IMPORTANT: For the food category, countries of origin (Italy, Japan, Mexico, etc.), cuisines (Italian cuisine, Japanese cuisine), ingredients (tomato, rice, cheese), and famous chefs are all VALID bridge nodes — do NOT flag any of these as wrong_domain.',
    mythology: 'IMPORTANT: For the mythology category, pantheon groups (e.g. "Twelve Olympians", "Aesir", "Vanir", "Titans") and mythology systems (e.g. "Greek mythology", "Norse mythology") are VALID bridge nodes — do NOT flag these as wrong_domain or abstract. Family relationships (parent, child, consort) between mythological figures are also expected connections.',
    mb_rock: 'IMPORTANT: For the Rock Music category, record labels (e.g. "Atlantic Records", "Columbia Records") and songs are VALID bridge nodes at easy and medium difficulty. At hard difficulty, musical influences (artist A influenced artist B) and songs bridging artists are the expected connection type — a well-known artist being signed to a major label is too_easy for hard. Artists from related genres (pop, R&B, soul) that have rock-crossover songs or were signed to rock labels are VALID — do NOT flag them as wrong_domain.',
    mb_hiphop: 'IMPORTANT: For the Hip-Hop category, record labels and songs are VALID bridge nodes. Artists from related genres (R&B, soul, pop) that have hip-hop connections are VALID — do NOT flag as wrong_domain.',
    mb_pop: 'IMPORTANT: For the Pop Music category, record labels and songs are VALID bridge nodes. Pop is a broad genre — artists from R&B, rock, dance, or electronic backgrounds that have pop hits are VALID.',
    mb_rnb: 'IMPORTANT: For the R&B / Soul category, record labels and songs are VALID bridge nodes. Soul, gospel, and neo-soul artists are all valid.',
    mb_country: 'IMPORTANT: For the Country Music category, record labels and songs are VALID bridge nodes. Bluegrass, Americana, and country-pop artists are all valid.',
    rock: 'IMPORTANT: For the Rock Music category, record labels (e.g. "Atlantic Records", "Columbia Records") and songs are VALID bridge nodes at easy and medium difficulty. At hard difficulty, musical influences (artist A influenced artist B) and songs bridging artists are the expected connection type — a well-known artist being signed to a major label is too_easy for hard. Artists from related genres (pop, R&B, soul) that have rock-crossover songs or were signed to rock labels are VALID — do NOT flag them as wrong_domain.',
    hiphop: 'IMPORTANT: For the Hip-Hop category, record labels and songs are VALID bridge nodes. Artists from related genres (R&B, soul, pop) that have hip-hop connections are VALID — do NOT flag as wrong_domain.',
    pop: 'IMPORTANT: For the Pop Music category, record labels and songs are VALID bridge nodes. Pop is a broad genre — artists from R&B, rock, dance, or electronic backgrounds that have pop hits are VALID.',
    rnb: 'IMPORTANT: For the R&B / Soul category, record labels and songs are VALID bridge nodes. Soul, gospel, and neo-soul artists are all valid.',
    country: 'IMPORTANT: For the Country Music category, record labels and songs are VALID bridge nodes. Bluegrass, Americana, and country-pop artists are all valid.',
  }
...
export async function evaluateAndSelectPuzzle(
  ...
  const domainNotes: Record<string, string> = {
    ...
    food: 'Countries of origin (Italy, Japan, Mexico), cuisines (Italian cuisine, Japanese cuisine), ingredients (tomato, rice, cheese), and famous chefs are all VALID.',
...
const CONNECTION_TYPES: Record<string, Record<string, string>> = {
  ...
  mb_rock:    { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  mb_hiphop:  { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  mb_pop:     { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  mb_rnb:     { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  mb_country: { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  rock:       { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  hiphop:     { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  pop:        { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  rnb:        { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  country:    { easy: 'record labels and songs', medium: 'record labels and songs', hard: 'record labels, songs, and musical influences' },
  ...
  mythology: { easy: 'mythology systems, pantheon groups (e.g. Twelve Olympians, Aesir), and family relationships', medium: 'mythology systems, pantheons, and family', hard: 'pantheons, family, and legend' },
  ...
  food: { easy: 'country of origin, cuisine, and ingredients', medium: 'origin, cuisine, and food categories', hard: 'origin, food categories, ingredients, and chef creators' },
  comics: { easy: 'publisher', medium: 'publisher and teams', hard: 'publisher, teams, creators, artistic influences, and movements' },
  tv: { easy: 'shared cast members, fictional characters, and genres', medium: 'cast, creators, genres, and fictional characters', hard: 'cast, creators, genres, networks, screenwriters, and fictional characters' },
